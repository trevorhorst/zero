...
void canvas_set_pixel(Canvas *canvas, uint32_t x_point, uint32_t y_point, CanvasColor color)
{
    uint32_t canvas_x = x_point / 8;
    uint32_t canvas_y = y_point * (canvas->width / 8);
    if(color == CanvasColor::BLACK) {
        canvas->image[canvas_x + canvas_y] &= ~(0x80 >> (x_point % 8));
    } else {
        canvas->image[canvas_x + canvas_y] |= (0x80 >> (x_point % 8));
    }
}
                            ...
                            uint32_t offset_x, uint32_t offset_y)
{
    ...
    for(y = 0; y < sprite->height; y++) {
        for(x = 0; x < (sprite->width / 8); x++) {
            ...
            if(sprite->invert) {
                canvas->image[canvas_x + canvas_y] = (bmp->pixel_data[bmp_x + bmp_y]) ^ 0xFF;
            } else {
                ...
                for(uint32_t i = 0; i < 8; i++) {
                    ...
                    if(bmp->pixel_data[bmp_x + bmp_y] & (0x80 >> i)) {
                        canvas_draw_point(canvas, canvas_x_point, canvas_y_point, CanvasColor::WHITE, size);
                    } else {
                        canvas_draw_point(canvas, canvas_x_point, canvas_y_point, CanvasColor::BLACK, size);
                    }
                }
            }
        }
